...
func RegisterService(dao *dao.Dao, appConf *config.AppConf) pkgGrpc.InitServers {
	grpcServer := pkgGrpc.InitServers(func(s *grpc.Server) {
		hello.RegisterHelloServer(s, NewHelloService(appConf.Logger.Sugar()))

		auth.RegisterAuthenticationServer(s, NewAuthService(dao.UserDao, appConf))
	})
	return grpcServer
}
