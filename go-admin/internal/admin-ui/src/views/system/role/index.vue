<template>
  <div class="container-card">
    <el-card class="container-card-left" shadow="always">
      <el-tabs>
        <el-tab-pane>
          <span slot="label"><svg-icon icon-class="menu1" class-name="role-menu" /> 应用树</span>
          <el-tree
            ref="appTree"
            v-loading="appTreeLoading"
            :props="{children: 'children',label: 'desc'}"
            :data="appTree"
            node-key="ID"
            :default-expanded-keys="[this.selectedAppId]"
            :default-checked-keys="[this.selectedAppId]"
            @node-click="handleAppTreeClick"
          />
        </el-tab-pane>
      </el-tabs>
    </el-card>
...
import { getAppTree } from '@/api/system/app'
...
export default {
  ...
  methods: {
    ...
    async getTableData() {
      this.loading = true
        try {
          this.params.applicationId = this.selectedAppId
    ...
    async getAppTree() {
      this.appTreeLoading = true
      try {
        const data = store.getters.permission_apps
        // this.apps = apps
        // const { data } = await getAppTree()
        this.appTree = data
        this.appTreeLoading = true
      } finally {
        this.appTreeLoading = false
      }
    },

    handleAppTreeClick(data) {
    ...
    create() {
      ...
      this.dialogFormData.applicationId = this.selectedAppId
    },

    // 修改
    update(row) {
      this.dialogFormData.applicationId = row.applicationId
    ...
    async getMenuTree() {
      ...
      try {
        const { data } = await getMenuTree(this.selectedAppId)
    ...
    async getApiTree() {
      ...
      try {
        const { data } = await getApiTree(this.selectedAppId)
    ...
    async updateRoleMenusById() {
      ...
      try {
        await updateRoleMenusById(this.roleId, { menuIds: ids, applicationId: this.selectedAppId })
      } finally {
        this.permissionLoading = false
      }
    ...
    async updateRoleApisById() {
      ...
      try {
        await updateRoleApisById(this.roleId, { apiIds: ids, applicationId: this.selectedAppId  })
      } finally {
        this.permissionLoading = false
      }
