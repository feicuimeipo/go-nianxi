...
func NewEtcd(ttl int64, client *clientv3.Client) IDiscovery {
	var r = EtcdServer{}
	r.cli = client
	r.ttl = ttl
	return &r
...
// 将服务地址注册到etcd中
func (l *EtcdServer) register(serverAddr string, serverName string) error {

	//var err error
	//与etcd建立长连接，并保证连接不断(心跳检测)
	ticker := time.NewTicker(time.Second * time.Duration(l.ttl))
	go func() {
		key := "/" + schema + "/" + serverName + "/" + serverAddr
		for {
			...
			if err != nil {
				...
				err = keepAlive(serverName, serverAddr, l.ttl, l.cli)
				if err != nil {
...
// 取消注册
func (l *EtcdServer) deRegister(serverAddr string, serverName string) error {
	if l.cli != nil {
		key := "/" + schema + "/" + serverName + "/" + serverAddr
