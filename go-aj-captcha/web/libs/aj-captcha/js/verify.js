...
;(function($, window, document,undefined) {
	...
	function checkPictrue(data,baseUrl,resolve,reject){
		$.ajax({
			type : "post",
			contentType: "application/json;charset=UTF-8",
			//url : baseUrl + "/captcha/check?captchaType=blockPuzzle",
    ...
    Slide.prototype = {
        ...
        refresh: function() {
			...
			this.isEnd = false;
			getPictrue(
				{captchaType:"blockPuzzle", clientUid: localStorage.getItem('slider'), ts: Date.now()},this.options.baseUrl,function (res) {
				if (res.code==0) {
					...
					_this.$element.find(".backImg")[0].src = './libs/captcha/images/default.jpg'
        ...
        refresh: function() {
			...
			getPictrue({captchaType:"clickWord", clientUid: localStorage.getItem('point'), ts: Date.now()},_this.options.baseUrl,function(res){
				if (res.code==0) {
					...
					_this.htmlDoms.back_img[0].src = './libs/captcha/images/default.jpg';
