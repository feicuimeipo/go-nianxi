...
func TestClickWordCaptchaService(t *testing.T) {
	...
	fmt.Println(len(runesArray))

	//count := 4
	//for {
	//	u := str[utils.RandomInt(0, size-1)]
	//	r, size := utf8.DecodeRuneInString(string(u))
	//	fmt.Printf("%c %v\n", r, size)
	//	//set[unicode.(u)] = true
	//	//if len(set) >= count {
	//	//	break
	...
	//cachekey := make([]string, 0, len(set))
	//for k := range set {
